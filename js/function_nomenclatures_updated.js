...
function showAlert(message, type = 'info') {
  const container = document.getElementById('alerts-container') || createAlertsContainer();
...
async function loadNomenclatures(page = 1, append = false) {
  ...
  try {
    ...
    const response = await fetch(`request/nomenclatures_paginated.php?${params}`);
    ...
    showAlert('Erreur lors du chargement des nomenclatures: ' + error.message, 'danger');
  } finally {
    isLoading = false;
...
function createNomenclatureRow(nomenclature) {
  ...
  row.innerHTML = `
    <td>
      <input type="checkbox" class="nomenclature-checkbox" value="${escapeHtml(nomenclature.id)}">
    </td>
    <td>${escapeHtml(nomenclature.code_equipement || '')}</td>
    <td>${escapeHtml(nomenclature.code_article || '')}</td>
    <td><strong>${escapeHtml(nomenclature.repere_equipement || '')}</strong></td>
...
function applyFilters() {
  ...
  const codeEquipementFilter = document.querySelector('#code-equipement-filter');
  const codeArticleFilter = document.querySelector('#code-article-filter');
  const repereEquipementFilter = document.querySelector('#repere-equipement-filter');
  const fabricantFilter = document.querySelector('#fabricant-filter');
  const typeFilter = document.querySelector('#type-filter');
  const designationArticleFilter = document.querySelector('#designation-article-filter');
  const uniteFilter = document.querySelector('#unite-filter');
  const posteTechniqueFilter = document.querySelector('#poste-technique-filter');
...
function startProgressiveExport(type, exportType) {
  ...
  if (exportType === 'selected') {
    const checkboxes = document.querySelectorAll('.nomenclature-checkbox:checked');
  ...
  if (typeof EventSource !== 'undefined') {
    ...
    eventSource.onerror = function(event) {
      ...
      const iframe = document.createElement('iframe');
      iframe.style.display = 'none';
      iframe.src = url.replace('export_nomenclatures_progressive.php', 'export_nomenclatures.php');
      document.body.appendChild(iframe);
    ...
    if (data.step === 'complete') {
      progressText.innerHTML = `
        <div class="text-success">
          ...
          ${data.details && data.details.total_processed ? data.details.total_processed + ' nomenclatures exportées' : ''}
          ${data.details && data.details.filename ? '<br>Fichier: ' + data.details.filename : ''}
        </div>
      `;
    } else if (data.step === 'error') {
      progressText.innerHTML = `
...
async function loadNomenclatureDetails(id) {
  try {
    ...
    const editModal = new bootstrap.Modal(document.getElementById('editNomenclatureModal'));
    editModal.show();
  } catch (error) {
...
function fillEditForm(nomenclature) {
  ...
  const fields = [
    ...
    'numero_poste', 'quantite', 'unite', 'poste_technique', 'metier',
...
function updateStatistics(total) {
  const totalElement = document.querySelector('[data-stat="total-nomenclatures"]');
  if (totalElement) {
...
async function updateDetailedStats() {
  try {
    ...
    if (stats.success) {
      ...
      const topFamilleElement = document.querySelector('[data-stat="top-famille"]');
      if (topFamilleElement && stats.top_famille) {
        topFamilleElement.textContent = stats.top_famille;
      }
      
      const maxFamilleElement = document.querySelector('[data-stat="max-famille"]');
...
function initNomenclaturesPage() {
  ...
  const filters = [
    '#code-equipement-filter', '#code-article-filter', '#repere-equipement-filter',
    '#fabricant-filter', '#type-filter', '#designation-article-filter',
    '#unite-filter', '#poste-technique-filter', '#metier-filter', '#source-filter'
  ];
  ...
  const exportFilteredExcelBtn = document.querySelector('#export-filtered-excel-btn');
